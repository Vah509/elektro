...
> Актуально станом на: 2026-09-06
>
> ⚠️ Розділ «СТРУКТУРА СТОРІНОК САЙТУ» та примітка про BaseLayout нижче
> застаріли станом на 2026-07-12: з `layouts-docs.md` (2026-07-21) відомо,
> що сайт мігрував на `BaseLayout.astro` + `ProductLayout.astro` для всіх
> сторінок. Цей розділ ще не звірено з реальним кодом — не покладатися
> на нього без перевірки.
...
### WorkPhotos.astro — адаптивна розкладка (оновлено 2026-09-06)

Компонент має дві різні розкладки залежно від ширини екрана:

- **Мобільний/планшет (< 901px)** — без змін з моменту створення:
  головне фото зверху на всю ширину (`aspect-ratio: 4/3`, `object-fit: cover`),
  мініатюри стрічкою знизу з горизонтальним скролом.

- **Десктоп (901px+)** — перероблено 2026-09-06:
  - Grid `2fr / 1fr` всередині контейнера `max-width: 1200px` (той самий
    контейнер, що і в решти контенту сайту — раніше блок розтягувався на
    всю ширину екрана, що виглядало важко).
  - Головне фото зліва, БЕЗ обрізки (`object-fit: contain`), вписане по
    висоті `max-height: 70vh` — не дає портретним фото ламати розкладку
    або створювати скрол сторінки.
  - Мініатюри справа сіткою 2 колонки, зі своїм внутрішнім скролом
    (`overflow-y: auto`), якщо фото багато і вони не вміщуються в 70vh.

Причина зміни: початкова розкладка (full-bleed фото на всю ширину з
`object-fit: cover`) візуально «тиснула» — особливо на широких десктопних
екранах. Підтверджено Вахтангом на живому сайті 2026-09-06.
...
*Проект: elektroschit.com.ua | context-project v3.1 (консолідований) | 2026-09-06*
